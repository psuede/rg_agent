...
async function addTelegramMessage(groupId, userId, content, isMedia, timestamp, reaperMessageId, reaperReplyTo) {
...
async function addTelegramMessageEntity(messageId, entityId) {
...
async function addTokenCall(groupId, userId, content, token, timestamp, messageId) {
  await db.raw(`INSERT INTO "TokenCall" ("groupid", "content", "timestamp", "contractaddress", "userid", "messageid") VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT DO NOTHING`,
    [groupId, content, timestamp, token, userId, messageId]);
}

async function markDeleted(reaperMessageId) {
